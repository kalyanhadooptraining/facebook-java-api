...
public class FacebookNamespaceContextTest {
	...
	public void testUse() throws Exception {
		...
		Element fqlQueryResponse = document.createElementNS(
				"http://api.facebook.com/1.0/", "fql_query_response");
		...
		assertEquals("No elements will be found because the processor "
				+ "doesn't yet know about the fbapi prefix", "0", count);
		xpath.setNamespaceContext(fnc);
		count = xpath.evaluate(xpathexpression, document);
		assertEquals("1 element will be found because the processor "
				+ "now knows about the fbapi prefix", "1", count);
	}
}
